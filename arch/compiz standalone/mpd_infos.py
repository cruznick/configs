...
# settings
mpd_dir = '/media/archivos/musica'							#mpd music directory
notify_osd = True								#enable/disable notify-osd 
conky = True									#enable/disable conky cover linking
cover_default = '/home/cruznick/conky/openbox/icons/Music_Player.png'	#default cover for notify-osd
...
def action(status):
	if status == 'play':
		bubble = 'notification-audio-play' + ' MPD Reproduciendo'
		notify(coverPath(),artist,album,title)
	elif status == 'pause':
		bubble = 'notification-audio-pause' + ' MPD Pausado'
	else:
		bubble = 'notification-audio-stop' + ' MPD Detenido'
